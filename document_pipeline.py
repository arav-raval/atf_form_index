...
def _index_remove_serial(idx: dict[str, Any], serial_norm: str) -> None:
    bys = idx.get("by_serial") or {}
    bys.pop(serial_norm, None)
...
class DocumentProcessResult:
    ...
    predicted_serial: str
    predicted_serial_raw: str
...
def process_pdf_document(
    ...
    *,
    ...
    """
    Full-document pipeline: classify → serial OCR using predicted year’s template.

    If ``json_path`` is omitted, uses ``<pdf_stem>.json`` beside the PDF.
    Merges a ``processing`` object into the JSON (creates minimal JSON if missing).
    """
    import classifier
    ...
    cr = classifier.classify(str(pdf_path), library, threshold=th)
    ...
    if pred_year and status != "ERROR":
        ex = serial_extract.extract_serial_from_pdf(
            pdf_path, pred_year, templates_dir, row_index=0
        )
    ...
    if gt is not None:
        ...
        proc["serial_match"] = serial_extract.normalize_serial(gt) == serial_norm if serial_norm else None
    ...
    save_json(jp, merged)
    log.info(
        "Processed %s → year=%s serial=%s (status=%s)",
        pdf_path.name,
        pred_year,
        serial_norm or "(empty)",
        status,
    )

    if update_index and serial_norm:
        idx = load_search_index(index_path)
        old = (doc.get("processing") or {}).get("predicted_serial")
        if old and old != serial_norm:
            _index_remove_serial(idx, str(old))
        _index_upsert(
            idx,
            serial_norm,
            str(jp),
...
def rebuild_index_from_json(
    ...
    """Scan JSON files for ``processing.predicted_serial`` and rebuild ``search_index``."""
    idx: dict[str, Any] = {"version": 1, "by_serial": {}}
    n = 0
    for d in directories:
        if not d.is_dir():
            continue
        for jp in d.glob("*.json"):
            ...
            sn = proc.get("predicted_serial") or ""
            if not sn:
                continue
            form = doc.get("form") or {}
            ...
            _index_upsert(
                ...
                str(jp.resolve()),
                str(pdf_path),
                str(proc.get("predicted_year") or ""),
                form.get("serial"),
            )
            n += 1
    save_search_index(idx, index_path)

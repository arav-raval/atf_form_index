...
def predict_image(
    ...
    Returns:
        List of dicts sorted by confidence (highest first). Each dict has:
            "is_serial"        – True if this is a serial-number page
            "form_year"        – e.g. "2016"  (None when is_serial is False)
            "page_within_form" – e.g. 3        (None when is_serial is False)
            "confidence"       – float 0-1
    """
    with open(label_map_path, "rb") as f:
        meta = pickle.load(f)
    idx_to_label: dict[int, object] = meta["idx_to_label"]
    ...
    for p, i in zip(top_probs, top_idxs):
        key = idx_to_label[i.item()]
        if key == "discard":
            results.append({
                "is_serial": False,
                "form_year": None,
                "page_within_form": None,
...
if __name__ == "__main__":
    ...
    for rank, r in enumerate(results, 1):
        if r["is_serial"]:
            desc = f"Serial-number page — form year {r['form_year']}, page {r['page_within_form']}"
        else:
